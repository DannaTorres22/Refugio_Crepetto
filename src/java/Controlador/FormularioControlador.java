        ...
        switch (opcion) {
            case 1:  //agregar registro
                if (formDAO.agregarRegistro()) {
                    request.setAttribute("MensajeExito", "El Formulario se registro");
            ...
            case 2:  //actualizar registro
                if (formDAO.actualizarRegistro()) {
                    request.setAttribute("MensajeExito", "El Formulario se actualizo");
                    masVO.setEstadoMascota("Adoptada");
                    masVO.setIdMascota(idMascota);
                    masDAO.actualizarEstado(masVO);
                } else {
            ...
            case 6:
                formVO = formDAO.consultarId(idFormulario);
                if (formVO != null) {
                    request.setAttribute("datosConsultadosss", formVO);
                    request.getRequestDispatcher("verEstado.jsp").forward(request, response);
                } else {
                    request.setAttribute("MensajeError", "No hay resultados que coincidan con tu busqueda");
